            ...
            <configuration>
                <archive>
                    <manifestEntries>
                        ...
                        <Plugin-Permissions>ui.menu,ui.notify</Plugin-Permissions>
                    </manifestEntries>
                </archive>
            </configuration>
        </plugin>
...
| `setConfig(String key, String value)` | Write a config value. **A null value is a silent no-op, not a delete** -- there is no unset API. |
...
| `-- plugin-permissions: cap1, cap2` | In practice, yes | Declared capability list for permission gating (e.g. `clipboard.read, ui.menu, ui.dock`). **Omitting this denies every gated API** -- the host builds a deny-all profile, so calls fail and raise a "Plugin Permission Denied" dialog. Declaring an unknown capability fails the load. |
...
-- plugin-permissions: ui.panel, ui.menu, bus.publish, bus.subscribe
...
> **Current renderer note:** events tied to pending widgets or not-yet-wired interactions are part of the shared event schema, but are not emitted by the current webview renderer. Today that includes `tree_context_menu`, `table_context_menu`, `table_header_context_menu`, `path_bar_navigate`, `drop`, `context_menu_action`, and `toolbar_input_changed`.
...
> **Lua limitation:** the Lua `ui.form()` builder only forwards
> `id`, `name`, `label`, `text`, `value`, `default`, and `options`. The `hint`,
> `host_id`, `port_id`, `host_value`, and `port_value` fields in the table below are
> **silently dropped from Lua** -- a `host_port` field built in Lua falls back to the
> renderer defaults (`host`, `port`, `22`). Those fields are effectively Java-only.
...
> **Not every name below currently ships an asset.** The names match the SDK's icon
> constants, but the renderer resolves only 18 of them. These eight have **no asset**
> and render as a broken image today -- avoid them until assets land:
> `terminal`, `go-up`, `go-down`, `tab-sessions`, `tab-files`, `tab-tools`,
> `tab-macros`, `sidebar-folder`.
