...
# Corrected during implementation: the original 7.0.2 pin was incompatible with
# typescript-language-server 6.0.0, because the TypeScript 7 Go rewrite ships no
# lib/tsserver.js. 6.0.3 is the newest release that still does.
[[artifact]]
id = "typescript"
version = "6.0.3"
...
- I-2 ghost project groups — CLOSED by 7730f8e (Rust) and f136b5a (frontend). `stop_session` now emits one session-level `LspStatus` in the new terminal state `stopped` (`document_id: None`, delivered app-wide like diagnostics, because a popped-out Problems window owns none of the session's documents), and `problems-store.js` drops the session record on it, taking the group with it. The clean fix rather than the frontend prune: a session whose last state was `failed` publishes no further per-document status, so only the session itself can announce that it ended.
- L-1 signature help outliving its call — CLOSED by f136b5a. `overlayField` drops a carried-forward signature overlay when the transaction DELETED across its anchor (strict on both sides, since the anchor is zero-width), so deleting the call closes the hints while typing or backspacing inside it still keeps them.
- L-2 dwell-hover stealing the surface from signature help — CLOSED by f136b5a. `handlePointerMove` stands down (and cancels its dwell timer) while a signature overlay is visible. An explicit Show Hover still replaces it: a gesture wins, where the pointer happens to rest does not.
- N-1 chooser context previews are partial by design: a row shows the target LINE only when this window already has that file open, and the containing directory otherwise. `docs/superpowers/specs/2026-08-24-light-editor-lsp-design.md:558` asks for a short context preview on every row; closing that needs either a preview field on the `lsp_definition` payload (Rust) or a bounded lazy read for the highlighted row only. Recorded here because until now it lived only in a code comment in `app/features/editor/lsp-navigation.js`.
- N-2 consolidation — CLOSED by f136b5a. (i) `problems-navigation.js` reveals through `editor-service.revealRange` and keeps no conversion of its own; it still OPENS through `openLocalFile` rather than `openLocalFileAt`, because it needs the "no local pane in this window" branch for its panel-host fallback. (ii) `app/features/editor/lsp-position.js` is now the single `offsetAt`/`positionAt`/`spanOf`/`lineTextAt` implementation, consumed by all six converting modules and covered by `scripts/tests/test_lsp_position.mjs`, whose source scan fails if any of them grows a private copy again.
- N-3 lsp-navigation split — CLOSED by f136b5a. `lsp-navigation-history.js` owns the bounded per-window stacks and what a location is; `lsp-navigation-chooser.js` owns the candidate list's StateField and DOM; `lsp-navigation.js` is the orchestration between them, the definition request and editor-service.
- L-3 Markdown extraction — CLOSED by f136b5a. `app/features/editor/lsp-markdown.js` holds the normalizer and the segment renderer; `lsp-tooltips.js` delegates to it and is back to being an overlay state machine.
Manual-matrix notes for the hands-on pass (Steps 3–5), so these are recognized
rather than rediscovered:

- During the trust-revocation scenario, watch the Problems groups: the session-keyed group is deleted by the `stopped` status, and the per-document `untrusted` statuses that follow carry no session id, so they can re-create a group for the same project under its adapter-keyed identity. Confirm whether "revoked project still listed as untrusted" is the presentation the owner wants, or whether those post-stop statuses should be suppressed in the Problems store.
- Back toward a document another WINDOW owns consumes its history entry (that window is focused, and the step completes). Deliberate — leaving the entry would make every further Back focus the same window forever — but it is a one-line flip in `lsp-navigation.js`'s `step` if it feels wrong in practice.
- A popped-out Problems window opens the file without selecting the range (the `open-in-editor` host action carries a path and no range), and the definition chooser previews the target line only for files the window already has open. Both are the recorded degradations above, not new bugs.

- RELEASE WIRING (blocking for shippability): `.github/workflows/release.yml` builds via `cargo tauri build --target universal-apple-darwin` and never invokes the Makefile LSP staging, so every released DMG currently ships `Contents/Resources/lsp/` with only `.gitkeep` — LSP silently unavailable (fails closed) in all shipped builds; also unsolved for the x86_64 slice of a universal build. Needs a CI decision from the owner.
- Runtime hashes only program + `required_files` (5 files); the other ~139 receipt-recorded files are integrity-anchored by the app bundle's outer signature — accepted trust model, stated for the record.
