...
- N-2 consolidation follow-up, in two parts: (i) point `app/features/problems/problems-navigation.js`'s private `offsetAt`/`reveal` at `editor-service.revealRange`/`openLocalFileAt` — verified byte-equivalent behavior today, so this is the cheapest moment to dedup; (ii) extract the seven copies of the LSP position ↔ CodeMirror offset conversion (`editor-service.js`, `lsp-completion.js`, `lsp-completion-apply.js`, `lsp-diagnostics.js`, `lsp-tooltips.js`, `lsp-navigation.js`, `problems-navigation.js`) into one shared position helper beside `lsp-uri.js`, for the same reason URI conversion lives in exactly one place.
- N-3 split seams for `app/features/editor/lsp-navigation.js` next time it is opened: the history machinery (~90 lines: capture, bounded stacks, step) and the chooser DOM (~40 lines) are both self-contained.
