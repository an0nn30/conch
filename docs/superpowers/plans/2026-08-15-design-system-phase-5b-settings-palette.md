...
## A product decision this phase forces

Our settings use **iOS-style switches** (`makeSwitch`, 11 uses) for booleans; the reference uses **checkboxes** everywhere and has no switch control at all. Since the goal is that the two windows read as the same application, this plan converts switches to `.tl-check`. Toggle groups (`makeToggleGroup`, 5 uses) have no reference equivalent either, but they encode a 3-way choice rather than a boolean — keep them, restyled. **The controller must confirm the switch→checkbox change with the user before Task 2 starts**; if the user prefers keeping switches, restyle `.tl-switch` on tokens instead and note the deliberate divergence in METRICS.md.
...
Replace `renderDialogShell`'s hand-built `.ssh-overlay#settings-overlay` + `.ssh-form.settings-dialog` with `tlDialog.open({ title: 'Settings', size: 'lg', body, buttons })`. Buttons, in the reference's order: `Cancel` (secondary), `Apply` (secondary, **disabled until there are pending changes** — wire it to the existing dirty state in `features/settings/store.js`), `OK` (primary). The `?` help button sits bottom-left; the shell's footer is right-aligned, so add a `.tl-dialog__footer` left-slot for it in `components/dialog.css` (a small, general addition — the reference's dialogs use it too) rather than a settings-only hack. Delete the priority-210 Escape registrations at `settings.js` :196-205 and :225-235 — the shell owns Escape. Keep `openInWindow`'s standalone path working: it has no overlay, so it keeps its own shell but must use the same inner markup and classes.
...
`makeInput` (:81-92) → `.tl-input`; every `<select>` (5 sites listed in the anchors) → `tlCombo.attach`; numeric inputs → `tlSpinner.attach`; `addRow` (:24-44) → `.tl-settings__row` with the label column matching the reference. `makeSwitch` (:128-140, 11 uses) → `.tl-check` **only if the controller confirmed the switch→checkbox decision**; otherwise restyle `.tl-switch` on tokens. `makeToggleGroup` (:94-126) keeps its 3-way semantics, restyled on tokens.
