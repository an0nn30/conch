//! TermLab-specific configuration: keyboard shortcuts and UI preferences.
...
impl Default for TermLabConfig {
    fn default() -> Self {
        Self {
...
/// search_paths = ["~/.config/termlab/plugins"]
/// ```
///
...
mod tests {
    ...
    fn check_for_updates_defaults_to_true() {
        let config = TermLabConfig::default();
        assert!(config.check_for_updates);
    }

    #[test]
    fn check_for_updates_survives_round_trip() {
        let mut config = TermLabConfig::default();
        config.check_for_updates = false;
        let toml = toml::to_string(&config).unwrap();
        let parsed: TermLabConfig = toml::from_str(&toml).unwrap();
        assert!(!parsed.check_for_updates);
    }

    #[test]
    fn check_for_updates_missing_defaults_true() {
        let parsed: TermLabConfig = toml::from_str("").unwrap();
        assert!(parsed.check_for_updates);
    }
