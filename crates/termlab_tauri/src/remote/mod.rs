...
//!
//! All SSH/SFTP/transfer/tunnel logic is delegated to `termlab_remote`.
...
use termlab_remote::callbacks::{RemoteCallbacks, RemotePaths};
...
const BOOTSTRAP_END_MARKER: &str = "__TERMLAB_BOOTSTRAP_END__";
...
/// Bridges `termlab_remote::callbacks::RemoteCallbacks` to Tauri events and
...
fn desktop_remote_paths() -> RemotePaths {
    ...
    RemotePaths {
        ...
        config_dir: termlab_core::config::config_dir().join("remote"),
...
async fn establish_ssh_session(
    ...
    // Build callbacks and connect via termlab_remote.
    ...
    let (ssh_handle, channel) =
        termlab_remote::ssh::connect_and_open_shell(server, credentials, callbacks, &paths)
            .await
