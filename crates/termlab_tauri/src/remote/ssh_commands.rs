...
pub(crate) fn ssh_write(
    ...
    if data.as_bytes().contains(&0x1b) {
        log::debug!(
            "[termlab-keydbg] ssh_write pane={} len={} hex={}",
            pane_id,
            data.len(),
...
pub(crate) async fn ssh_get_pane_cwd(
    ...
    let (stdout, _stderr, exit_code) = match termlab_remote::ssh::exec(&handle, cmd).await {
        Ok(v) => v,
        Err(e) => {
            log::warn!(
...
pub(crate) async fn ssh_open_channel(
    ...
    let channel = termlab_remote::ssh::open_shell_channel(&ssh_handle, cols, rows)
        .await
    ...
    let task = tokio::spawn(async move {
        let exited = termlab_remote::ssh::channel_loop(channel, input_rx, output_tx).await;
...
fn credentials_from_vault_account(account: &termlab_vault::VaultAccount) -> SshCredentials {
    match &account.auth {
        termlab_vault::AuthMethod::Password(pw) => SshCredentials {
        ...
        termlab_vault::AuthMethod::Key { path, passphrase } => SshCredentials {
            username: account.username.clone(),
...
mod tests {
    ...
    fn credentials_from_vault_password_account() {
        let account = make_vault_account(
            "deploy",
            "Deploy Account",
            termlab_vault::AuthMethod::Password("s3cret".into()),
        );
    ...
    fn try_vault_credentials_returns_creds_when_linked() {
        ...
        let account_id = mgr
            .add_account(
                "Deploy".into(),
                "deploy".into(),
                termlab_vault::AuthMethod::Password("pw123".into()),
            )
            .unwrap();
        let vault: VaultState = Arc::new(Mutex::new(mgr));
