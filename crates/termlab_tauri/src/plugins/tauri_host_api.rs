...
impl HostApi for TauriHostApi {
    ...
    fn set_config(&self, key: &str, value: &str) {
        ...
        let path = dir.join(format!("{key}.json"));
        if let Err(e) = termlab_core::config::atomic_write(&path, value.as_bytes()) {
            log::error!(
    ...
    fn get_theme(&self) -> Option<String> {
        ...
        let dark_mode = !matches!(
            ...
            termlab_core::config::AppearanceMode::Light
        );
        Some(
            serde_json::json!({
    ...
    fn exec_active_session(&self, command: &str) -> Option<String> {
        ...
        if let Some(ssh_handle) = ssh_handle {
            let result =
                tauri::async_runtime::block_on(termlab_remote::ssh::exec(&ssh_handle, command));
