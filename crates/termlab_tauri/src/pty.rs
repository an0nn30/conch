...
fn resolved_shell(shell: &termlab_core::config::TerminalShell) -> (Option<&str>, &[String]) {
...
pub(crate) fn write_to_pty(
    ...
    if data.as_bytes().contains(&0x1b) {
        log::debug!(
            "[termlab-keydbg] write_to_pty pane={} len={} hex={}",
            pane_id,
            data.len(),
...
mod tests {
    ...
    fn resolved_shell_empty_program_uses_default_shell() {
        let shell = termlab_core::config::TerminalShell::default();
    ...
    fn resolved_shell_uses_configured_program_and_args() {
        let shell = termlab_core::config::TerminalShell {
