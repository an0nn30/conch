...
//!
//! Handles creating new TermLab windows with proper sizing, decorations, theme,
...
pub(crate) fn appearance_to_theme(
    mode: &termlab_core::config::AppearanceMode,
) -> Option<tauri::Theme> {
    match mode {
        termlab_core::config::AppearanceMode::Dark => Some(tauri::Theme::Dark),
        termlab_core::config::AppearanceMode::Light => Some(tauri::Theme::Light),
        termlab_core::config::AppearanceMode::System => None,
    }
}
...
pub(crate) fn create_window_with_label_titled<R: tauri::Runtime>(
    ...
    let (w, h) = crate::initial_window_px(
        &user_cfg.window.dimensions,
        &persisted.window_metrics,
        user_cfg.resolved_terminal_font(),
