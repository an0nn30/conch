...
(function (exports) {
  ...
  function showInApp(opts) {
    ...
    toast.className = 'termlab-toast termlab-toast-' + level;
    ...
    if (opts.action && opts.action.label) {
      actionHtml = `<div class="termlab-toast-actions"><button class="termlab-toast-action-btn">${esc(opts.action.label)}</button></div>`;
    }

    toast.innerHTML = `
      <div class="termlab-toast-header">
        <span class="termlab-toast-icon">${icon}</span>
        <span class="termlab-toast-title">${esc(opts.title)}</span>
        <button class="termlab-toast-close">\u2715</button>
      </div>
      ${opts.body ? `<div class="termlab-toast-body">${esc(opts.body)}</div>` : ''}
      ${actionHtml}
    `;

    toast.querySelector('.termlab-toast-close').addEventListener('click', () => dismiss(toast));

    if (opts.action && opts.action.callback) {
      const actionBtn = toast.querySelector('.termlab-toast-action-btn');
      if (actionBtn) {
