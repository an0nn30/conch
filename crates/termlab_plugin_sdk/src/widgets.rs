...
pub enum Widget {
    ...
    PathBar {
        id: String,
        /// Path segments (e.g., ["~", "projects", "termlab"]).
        segments: Vec<String>,
    },
...
/// `termlab_plugin_event()` on the plugin's thread.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(tag = "type", rename_all = "snake_case")]
...
/// Top-level event delivered to `termlab_plugin_event()`.
///
...
mod tests {
    ...
    fn widget_path_bar_roundtrip() {
        let w = Widget::PathBar {
            id: "path".into(),
            segments: vec!["~".into(), "projects".into(), "termlab".into()],
        };
        if let Widget::PathBar { segments, .. } = roundtrip(&w) {
            assert_eq!(segments, vec!["~", "projects", "termlab"]);
        } else {
            panic!("Wrong variant");
        }
    }
