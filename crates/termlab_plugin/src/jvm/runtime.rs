...
pub struct PluginMeta {
    ...
    pub plugin_type: termlab_plugin_sdk::PluginType,
    pub panel_location: termlab_plugin_sdk::PanelLocation,
}
...
static SDK_JAR_BYTES: &[u8] = include_bytes!("../../../../java-sdk/build/termlab-plugin-sdk.jar");
...
impl JavaPluginManager {
    ...
    fn probe_jar_metadata(&mut self, jar_path: &Path) -> Result<PluginMeta, LoadError> {
        ...
        // Call getInfo().
        let info_obj =
            match env.call_method(&plugin_obj, "getInfo", "()Ltermlab/plugin/PluginInfo;", &[]) {
                Ok(v) => match v.l() {
                    Ok(o) => o,
                    Err(e) => {
                        describe_java_exception(&mut env);
...
fn java_plugin_thread(
    ...
    if meta.plugin_type == termlab_plugin_sdk::PluginType::ToolWindow {
    ...
    while let Some(mail) = mailbox.blocking_recv() {
        match mail {
            ...
            PluginMail::BusEvent(msg) => {
                let event = termlab_plugin_sdk::PluginEvent::BusEvent {
...
fn read_plugin_info(env: &mut JNIEnv, info: &JObject) -> Result<PluginMeta, LoadError> {
    ...
    let plugin_type = match plugin_type_str.as_str() {
        "tool_window" | "panel" => termlab_plugin_sdk::PluginType::ToolWindow,
        _ => termlab_plugin_sdk::PluginType::Action,
    };
    let panel_location = match panel_location_str.as_str() {
        "left" => termlab_plugin_sdk::PanelLocation::Left,
        "right" => termlab_plugin_sdk::PanelLocation::Right,
        "bottom" => termlab_plugin_sdk::PanelLocation::Bottom,
        _ => termlab_plugin_sdk::PanelLocation::None,
    };
...
/// Register native method implementations for `termlab.plugin.HostApi`.
fn register_host_natives(env: &mut JNIEnv) -> Result<(), LoadError> {
    let class = env.find_class("termlab/plugin/HostApi").map_err(jni_err)?;
