...
const MAGIC: &[u8; 8] = b"TRMLBVLT";
/// Pre-rebrand magic. Vaults written by Conch builds are still readable;
/// the next save rewrites the file with the new magic.
const LEGACY_MAGIC: &[u8; 8] = b"CONCHVLT";
...
mod tests {
    ...
    #[test]
    fn decrypt_accepts_legacy_conch_magic() {
        // Vaults written by pre-rebrand Conch builds carry CONCHVLT magic and
        // must keep decrypting after the rename.
        let vault = make_test_vault();
        let mut encrypted = encrypt_vault(&vault, b"password").unwrap();
        encrypted[..8].copy_from_slice(LEGACY_MAGIC);
        let decrypted = decrypt_vault(&encrypted, b"password").unwrap();
        assert_eq!(decrypted.accounts.len(), vault.accounts.len());
    }
